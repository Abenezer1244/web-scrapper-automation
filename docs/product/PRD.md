...
**Provider:** Tracerfy — API base `https://tracerfy.com`, Bearer auth, batch endpoint `POST /v1/api/trace/`, webhook-delivered CSV results.
...
**Scope notes (decisions locked 2026-04-10 before coding started, revised 2026-04-10 after reading Tracerfy API docs):**

1. **Batch mode only** — Sprint 4 uses `POST /v1/api/trace/` (async, 1 credit per row uploaded for normal trace, 2 for advanced, delivered via webhook). The synchronous `POST /v1/api/trace/lookup/` endpoint (5 credits per hit, 0 on miss) is deferred to Sprint 5+ as an "instant lookup" UI premium feature — it has different unit economics and requires separate Stripe metering.

2. **Advanced-trace routing for entity grantors** — Records where the grantor name contains `LLC`, `TRUST`, `ESTATE`, `INC`, `LP`, `LLP`, `COMPANY`, or similar entity tokens get submitted with `trace_type=advanced` (Tracerfy finds the real human owner from the address alone, 2 credits). All other records get `trace_type=normal` (1 credit, uses the grantor name we supply). This blended cost model averages ~$0.026 wholesale per record when ~30% of records are entity grantors (common in WA probate/pre-foreclosure). Gross margin at $0.08 retail stays above 65%.

3. **Skip-trace dispatcher** — Tracerfy rate limits batch POSTs to **10 requests per 5-minute window per account**. A single dispatcher (new Celery Beat task, tick every 5 min) consolidates all pending skip-trace rows across all user jobs into up to 2 batch POSTs per tick. Individual scrape jobs do NOT call Tracerfy directly — they enqueue rows into `pending_skip_trace_rows` and return. The dispatcher drains the queue, submits in batches, and correlates results back to jobs via `skip_trace_queues.job_id`.

4. **Webhook receiver** — Tracerfy delivers results by POSTing to a webhook URL configured in the account profile (not registered via API). The URL is `POST /webhooks/tracerfy/{SECRET}` on our FastAPI app, where `{SECRET}` is a random 32-byte URL-safe string from `TRACERFY_WEBHOOK_SECRET`. The endpoint validates the secret, fetches the CSV from the payload's `download_url`, parses it, matches rows back to `Result` rows by `(address, city, state)`, upserts phone/email. Idempotent by `queue_id`.

5. **Metered billing model** — per §5.4 pricing table. Starter: shown with upsell UI, backend rejects with 402. Pro: $0.08/trace. Business: 1,000 bundled + $0.08/trace overage. Agency: 2,000 bundled + $0.05/trace overage. Users opt-in via a checkbox on scraper config (default OFF to avoid accidental charges).

6. **Eligibility filter** — Only records with a non-null `property_address` are sent to Tracerfy. No point tracing records we can't deliver anyway, and it reduces wasted credits.

7. **90-day parcel-level cache** — `skip_trace_cache` table keyed on `(property_address, city, state)` hash. Re-scraping the same parcel across daily jobs reads from cache, does not re-bill the user. TTL: 90 days. Saves an estimated 50-70% of Tracerfy calls on repeat scrapes.

8. **TCPA DNC/litigator scrub deferred to Sprint 5** — Tracerfy's DNC scrub endpoint exists (`POST /v1/api/dnc/scrub/`, 1 credit per phone checked) but integration is out of scope for Sprint 4. CSV export includes a disclaimer footer: "Verify numbers against the National DNC Registry before calling. BridgeLeads does not currently pre-scrub phone numbers against DNC or TCPA litigator lists." Interestingly, the Tracerfy batch trace response already includes a `dnc` flag per phone in the JSON response, so we can opportunistically capture that even in Sprint 4 and expose it in the export — it just doesn't replace a formal DNC scrub step.

| # | Task | Detail | Done When |
|---|------|--------|-----------|
| 4.1 | Evaluate skip trace providers | ✅ DONE 2026-04-10. Tracerfy selected (see §5.1, §5.4, and Tracerfy API docs at docs/vendor/tracerfy-api.md). BatchData is Phase-2 upgrade path. | Provider selected |
| 4.2 | DB schema + provider module + webhook receiver | Alembic migration `014_add_skip_trace.py`: add `phone`, `phone_type`, `phone_dnc_flag`, `email`, `skip_trace_status`, `skip_trace_attempted_at` to `results`. Create `skip_trace_cache` (address-hash key, 90-day TTL), `skip_trace_queues` (tracerfy_queue_id, job_id, status, download_url), `pending_skip_trace_rows` (job_id, row data, enqueued_at). Build `src/scrapers/enrichment/skip_trace.py` (`build_payload`, `submit_batch`, `ingest_webhook`, `classify_grantor_as_entity`). Build `src/api/routes/webhooks.py::tracerfy_webhook`. Settings: `TRACERFY_API_TOKEN`, `TRACERFY_WEBHOOK_SECRET`, `SKIP_TRACE_ENABLED`. | Unit test builds payload correctly for 10 records; burns ~10 real credits submitting to `/v1/api/trace/`; webhook endpoint receives completion, parses CSV, writes phone/email to Result rows. ≥60% phone hit rate. |
| 4.3 | Skip-trace dispatcher (Celery Beat) | New task `workers/skip_trace_dispatcher.py::dispatch_pending_skip_trace` scheduled every 5 min via Beat. Drains `pending_skip_trace_rows`, groups by `trace_type` (normal/advanced), submits up to 2 batch POSTs per tick (rate-limit headroom). Updates `skip_trace_queues` row with returned `queue_id`. | Dispatcher visible in Flower; completes a round-trip on staging without 429s. |
| 4.4 | Worker pipeline integration | `_run_inline_enrichment` (in `workers/tasks.py`), after GIS + King assessor fallbacks: for each record with non-null property_address AND user's plan has skip trace enabled AND cache miss, classify grantor entity-vs-individual, insert row into `pending_skip_trace_rows`, write back any cache hits directly. Job log shows `"Enqueued N rows for skip trace (M from cache)"`. | End-to-end Pierce job enqueues rows; dispatcher submits batch; webhook completes; Result rows updated; job log confirms counts. |
| 4.5 | CSV/Excel export columns + DNC disclaimer | `DataExporter` emits `phone`, `phone_type`, `phone_dnc_flag`, `email`, `skip_trace_status`. Appends DNC disclaimer as a footer row in the CSV until Sprint 5 TCPA scrub ships. | Visible in downloaded CSV; disclaimer present. |
| 4.6 | Stripe metered billing | New Stripe Product "Skip Trace Lookup" with three Prices ($0.08 Pro, $0.08 Business-overage, $0.05 Agency-overage). Dispatcher reports usage via Stripe Meter Events API after each successful batch (not per-row — after webhook ingest to avoid charging for errored batches). Webhook handler for `invoice.created`. | Test charge lands on Stripe dashboard for 1 trace in staging. |
| 4.7 | Plan gating + opt-in UI (backend) | `check_plan_enforcement` rejects skip-trace for Starter with 402 Payment Required. `scraper_configs.skip_trace_enabled` boolean field (default False). API accepts the flag on POST/PUT /scrapers endpoint. | Starter user sees 402 on skip-trace attempt; Pro user's opt-in persists across scraper config edits. |
| 4.8 | Frontend changes (separate repo) | Scraper config form: add skip-trace opt-in checkbox, disabled for Starter with upsell tooltip "Upgrade to Pro to unlock skip trace ($0.08/lookup)". Results table: add `phone`, `email` columns with click-to-copy. Billing page: skip-trace usage meter (used this month / bundled quota / overage). | Columns visible; meter updates; Starter sees upsell state. |
| 4.9 | Production verification | Run Pierce + King probate (or pre_foreclosure) with skip trace enabled against an internal test account. Confirm ≥60% phone hit rate AND ≥25% email hit rate across real records. Measure Tracerfy latency (queue → webhook). Measure cache hit rate on day-2 re-scrape. | Verification logs pasted into Sprint 4 memory file. Rates meet gate. |
...
| 2026-04-10 | 1.3 | Engineering | **Sprint 4 architecture locked after reading real Tracerfy API docs.** Batch-only scope (instant lookup deferred). Advanced-trace routing for entity grantors (LLC/Trust/Estate) adds 2-credit path for ~30% of records, blended cost ~$0.026/record. New skip-trace dispatcher task required due to Tracerfy rate limit (10 batch POSTs per 5-min window) — no direct Tracerfy calls from scrape jobs. New webhook receiver `POST /webhooks/tracerfy/{SECRET}` required for result delivery. Starter tier changes from "excluded entirely" to "shown with upsell UI, rejected 402 on backend" — better conversion funnel. Sprint 4 task list expanded from 8 to 9 (added dispatcher task 4.3). §6 Sprint 4 section rewritten. |
