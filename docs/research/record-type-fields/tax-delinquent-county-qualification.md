...
- **`bill_year`** — integer, clamped `[1900, current_year + 1]`. The `current_year + 1` upper bound is **input sanitization** (reject absurd values), NOT a statement that next-year bills are delinquent. The delinquency rule is separate and stricter: **the scraper must exclude current-year and future rows** (`bill_year >= current_year`) before emitting — a delinquency must be ≥1 cycle old. Do not conflate the clamp with the rule.
