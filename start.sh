...
if [ "$RAILWAY_SERVICE_NAME" = "worker" ]; then
  echo "Starting Celery worker with xvfb-run for headed Playwright..."
  # xvfb-run wraps the command with a virtual X display.
  # This lets Playwright run in headed mode, fixing EagleWeb JS redirects.
  exec xvfb-run --auto-servernum --server-args="-screen 0 1280x800x24 -ac" \
