...
from src.utils.address_intel import _street_is_placeholder, compute_owner_flags

_REAL_MAILING = "1234 REAL ST, EVERETT, WA 98208"


def test_detects_the_measured_placeholder_shapes():
    assert _street_is_placeholder("UNKNOWN, SNOHOMISH WA") is True
    assert _street_is_placeholder("UNKNOWN UNKNOWN, LAKE STEVENS WA 98258") is True
    assert _street_is_placeholder("UNKNOWN UNKNOWN UNKNOWN, GRANITE FALLS WA 98252") is True
...
def test_a_real_street_is_never_a_placeholder():
    ...
    assert _street_is_placeholder("123 UNKNOWN RD, TACOMA, WA 98402") is False
    assert _street_is_placeholder("8822 2ND AVE S") is False
    assert _street_is_placeholder(None) is False
    assert _street_is_placeholder("") is False
